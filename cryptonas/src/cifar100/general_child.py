...
from src.cifar100.image_ops import conv
from src.cifar100.image_ops import fully_connected
from src.cifar100.image_ops import batch_norm
from src.cifar100.image_ops import batch_norm_with_mask
from src.cifar100.image_ops import relu
from src.cifar100.image_ops import max_pool
from src.cifar100.image_ops import global_avg_pool
...
class GeneralChild(Model):
  ...
  def _enas_layer(self, layer_id, prev_layers, start_idx, out_filters, is_training):
    ...
    if self.whole_channels:
	  ...
	  # client online cost
      counts = {}
      counts[tf.equal(count, 0)] = lambda: tf.constant(3)
      counts[tf.equal(count, 1)] = lambda: tf.constant(3)
      counts[tf.equal(count, 2)] = lambda: tf.constant(5)
      counts[tf.equal(count, 3)] = lambda: tf.constant(5)
      filter_size = tf.case(counts, exclusive=True)
  ...
  def _fixed_layer(
    ...
    if self.whole_channels:
      ...
      if count in [0, 1, 2, 3]:
        ...
        ## Removing 1x1 blocks in cryptonas
    ...
    if layer_id > 0:
      ...
      with tf.variable_scope("skip"):
        ...
        ## removing skip connection ReLUs
        #out = tf.nn.relu(out)
        out = tf.nn.conv2d(
